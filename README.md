...
它和 DevC `HostData::HostGimbalTarget` 同布局，包含 roll/pitch/yaw、角速度和角加速度。模块只使用 `rol`/`yaw` 及其速度、加速度；`pit` 字段只用于匹配 DevC 数据布局。
...
`host/target_euler` 的 `rol == 0 && yaw == 0` 表示 Aimer 不接管云台。

## 坐标和符号

`host/gimbal_quat` 来自 WebotsCamera 发布的相机姿态。控制侧命令坐标系是 `x` 右、`y` 前、`z` 上。

`host/target_euler` 使用 DevC HostData 字段名；当前两轴云台只使用 `rol` 和 `yaw`，`pit` 保留为 0。机械俯仰轴是右手系 `+X` 轴，对应 ZYX 欧拉角 roll；yaw 是 `+Z` 轴，前向为 0、左转为正。
...
这个取反只用于匹配 Webots 关节轴方向，不改变 `host/target_euler` 的语义。
...
当前 Webots 云台不使用固定 pitch 重力补偿；惯量前馈和两轴摩擦补偿是模型固定项。改 world 里的质量、重心、摩擦或关节轴以后，需要重新标定这些固定项。
