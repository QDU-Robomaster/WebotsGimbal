...
它和 DevC `HostData::HostGimbalTarget` 同布局，包含 roll/pitch/yaw、角速度和角加速度。模块只使用 pitch/yaw 及其速度、加速度；roll 字段保留为 ABI 对齐。

当 `pitch == 0 && yaw == 0` 时，语义与 DevC `HostData` 一致：上位机不接管云台。模块会清空当前目标、复位 PID，并让控制线程把两轴力矩置零。
...
`host/target_euler` 的 `pitch == 0 && yaw == 0` 表示上游不要接管云台。

## 坐标和符号

`gimbal/rotation` 来自 WebotsCamera 发布的相机姿态。控制侧命令坐标系是 `x` 右、`y` 前、`z` 上。

当前 Webots world 的传感器安装零位下，目标正前方时 `gimbal/rotation` 的 ZYX yaw 约为 `-pi/2`；模块在比较 yaw 前加固定 `pi/2` 偏移，使反馈 yaw 与 `host/target_euler.yaw` 的前向零位对齐。pitch 仍使用同一姿态里的 ZYX pitch。
